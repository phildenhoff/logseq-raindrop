...
describe("getLeftAndParentBlocks", () => {
  it("returns the right values when sibling is false and before is true", async () => {
    ...
    const result = await getLeftAndParentBlocks(
      SIBLING,
      BEFORE,
      blocksMap,
      pagesMap,
  ...
  it("returns the right values when both sibling and before are true", async () => {
    ...
    expect(blockA?.left).toEqual({ id: 1, uuid: "_p1" });
  });
...
describe("updateBlockLeft", () => {
  it("updates the first child when sibling is false and before is true", async () => {
    /**
     * We have this page layout:
     * Page 1
     *   Block A
    ...
    updateBlockLeft(SIBLING, BEFORE, blocksMap, pagesMap, "_a", {
      id: 4,
      uuid: "_new",
    });
  ...
  it("updates the reference block when sibling and before are both TRUE", async () => {
    /**
     * We have this page layout:
     * Page 1
     *   Block A
